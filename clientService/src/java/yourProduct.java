 ...
 * @author Kris
 */
@WebServlet(urlPatterns = {"/yourProduct"})
public class yourProduct extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        //String uname = identityConnect.getUser(Integer.parseInt(request.getSession().getAttribute("uid").toString())).get("uname").toString();
    ...
    private java.util.List<net.java.dev.jaxb.array.StringArray> getAllProductByUserID(int uid) {
        // Note that the injected javax.xml.ws.Service reference as well as port objects are not thread safe.
        // If the calling of port operations may lead to race condition some synchronization is required.
        webservice.ProductWebService port = service.getProductWebServicePort();
        return port.getAllProductByUserID(uid);
    }
