...
public class identityConnect {
    ...
    public static JSONObject validateLogin(String username, String password) {
        JSONObject response = null;
        try {
            String utf8 = java.nio.charset.StandardCharsets.UTF_8.name();
            String query = String.format("username=%s&userpass=%s",
    ...
    public static JSONObject validateToken(String token, int id) {
        JSONObject response = null;
        try {
            String utf8 = java.nio.charset.StandardCharsets.UTF_8.name();
            String query = String.format("token=%s&uid=%d",
                    URLEncoder.encode(token, utf8),
                    Integer.parseInt(URLEncoder.encode(Integer.toString(id), utf8)));
            response = processRequest("/tokenServlet", query.getBytes()); 
